...
对于任意群 $G$ 以及 $X \sub G$, 令 $G$ 中所有包含 $X$ 的子群族设为 $\set{ H_i \supset X : i \in I }$, 则：
...
由于直积无非就是 $\operatorname{Mon}$ (幺半群范畴) 或 $\operatorname{Grp}$ (群范畴) 下的范畴积, 因此其亦直接满足了泛性质, 就引出了 [引理 1.3.2 (直积满足泛性质)](#引理_1.3.2_(直积满足泛性质)).
