...
- 环 $R$ 中单位 $a \in R$ 的左右逆元必定是重叠的, 因为 $ab = 1_R = ca$ 蕴含了 $b = 1_R b = (ca)b = c(ab) = c1_R = c$;
- 含幺环 $R$ 中所有单位的集合于乘法下构成群 $(R, \cdot)$.
...
- 设有含乘法幺元 $1_D \neq 0$ 的环 $D$, 且任意非零元皆为单位元, 则 $D$ 被称为 **除环 (division ring)**;
...
- 事实上为了衡量从环到域的差距, 所以分别定义出 单位元, 零因子 以及 幂零元的概念, 它们的集合有以下的包含关系：
  $$
  \begin{array}{c}
...
- 域 $\mathbb{F}$ 上的 $n \times n$ 矩阵的集合 $\op{M}_n(\mathbb{F})$ 构成非交换的含幺环, 其中该环上的单位便是非奇异矩阵 (可逆矩阵).
